...
的内部规划模板（Think模板）
我理解您的需求！除了系统提示词之外，我在实际工作中会在think工具中使用以下内部规划模板来组织PPT制作流程：
...
- [ ] 复古精致（中文：Source Han Serif SC；英文：Spectral + Quattrocento Sans）
- [ ] 活力未来（中文：抖音黑体 + MiSans；英文：BioRhyme + Archivo）
...
### 图表素材
- [ ] 图表1：[类型，如时间轴/柱状图] - 用途：[哪一页]
- [ ] 图表2：[类型，如饼图/流程图] - 用途：[哪一页]
...

### 图标素材
- [ ] Material Icons库中的图标：[图标名称]
...
...
### 布局选择（每页）
- 第1页：[封面布局，如Minimalist-Typography-Center-Focus]
- 第2页：[内容布局，如Vertical-Flow-Text-Top]
- 第3页：[内容布局，如Split-Tone-Image-Left-Text-Right]
...
...
2. 判断风格类型：
   - 正式/商务 → 商务风格（MiSans / Source Code Pro + Roboto Flex）
   - 复古/精致 → 复古风格（Source Han Serif SC / Spectral + Quattrocento Sans）
   - 活泼/未来 → 活力风格（抖音黑体 / BioRhyme + Archivo）
...
阶段1：信息收集（如果需要）
1. search - 搜索相关主题信息
2. visit_page - 访问重要网页获取详细内容
3. search_images - 搜索图片素材（最多6次）

阶段2：PPT初始化
1. initialize_design - 创建PPT框架
   - description: [内容概要+设计风格]
   - slide_name: [文件名]
   - title: [PPT标题]
   - slide_num: [总页数]
   - width: 1280
...
阶段3：页面生成
1. insert_page - 插入第1页（index: 1）
2. insert_page - 插入第2页（index: 2）
3. ...
4. insert_page - 插入第N页（index: N）

阶段4：优化调整（根据需要）
1. update_page - 修改某页内容
2. remove_pages - 删除不需要的页面
