...
${htmlContent}
  ...
  const handleDownloadPPT = useCallback(async () => {
    ...
    setIsDownloading(true);
    try {
      // Map locale to API format (zh -> zh-CN, en -> en-US)
      const apiLocale = locale === "zh" ? "zh-CN" : "en-US";
      const blob = await generatePPT(report.content, apiLocale);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `research-report-${getTimestamp()}.pptx`;
      document.body.appendChild(a);
      a.click();
      setTimeout(() => {
        try {
      ...
      toast.error(t("errorGeneratingPPT") || t("exportFailed"));
    } finally {
      setIsDownloading(false);
    }
  }, [reportId, getTimestamp, isDownloading, t, locale]);
