...
async def generate_ppt(request: GeneratePPTRequest):
    ...
    try:
        ...
        generated_file_path = final_state.get("generated_file_path")
        
        if not generated_file_path or not os.path.exists(generated_file_path):
            error_msg = f"Generated PPT file not found: {generated_file_path}"
            logger.error(error_msg)
            raise FileNotFoundError(error_msg)
        
        logger.info(f"Reading PPT file: {generated_file_path}")
        ...
        try:
            ...
            logger.info(f"Cleaned up generated PPT file: {generated_file_path}")
        except Exception as e:
            logger.warning(f"Failed to clean up PPT file {generated_file_path}: {e}")
