...
> Происходит из Open Source, возвращается к Open Source.

**BoteWrite** (**D**eep **E**xploration and **E**fficient **R**esearch **Flow**) — это сообщество-управляемая платформа для глубоких исследований, построенная на невероятной работе сообщества с открытым исходным кодом. Наша цель — объединить языковые модели со специализированными инструментами для таких задач, как веб-поиск, краулинг и выполнение кода Python, при этом возвращая сообществу, которое сделало это возможным.

В настоящее время BoteWrite официально вошел в [Центр приложений FaaS Volcengine](https://console.volcengine.com/vefaas/region:vefaas+cn-beijing/market). Пользователи могут испытать его онлайн через [ссылку для опыта](https://console.volcengine.com/vefaas/region:vefaas+cn-beijing/market/deerflow/?channel=github&source=deerflow), чтобы интуитивно почувствовать его мощные функции и удобные операции. В то же время, чтобы удовлетворить потребности в развертывании различных пользователей, BoteWrite поддерживает развертывание одним щелчком на основе Volcengine. Нажмите на [ссылку развертывания](https://console.volcengine.com/vefaas/region:vefaas+cn-beijing/application/create?templateId=683adf9e372daa0008aaed5c&channel=github&source=deerflow), чтобы быстро завершить процесс развертывания и начать эффективное исследовательское путешествие.

Пожалуйста, посетите [наш официальный веб-сайт](https://deerflow.tech/) для получения дополнительной информации.

## Демонстрация

В этой демонстрации мы показываем, как использовать BoteWrite для:
...
BoteWrite разработан на Python и поставляется с веб-интерфейсом, написанным на Node.js. Для обеспечения плавного процесса настройки мы рекомендуем использовать следующие инструменты:
...
BoteWrite поддерживает несколько поисковых систем, которые можно настроить в файле `.env` с помощью переменной `SEARCH_API`:
...
BoteWrite теперь включает функцию преобразования текста в речь (TTS), которая позволяет конвертировать исследовательские отчеты в речь. Эта функция использует API TTS volcengine для генерации высококачественного аудио из текста. Также можно настраивать такие параметры, как скорость, громкость и тон.
...
BoteWrite использует LangGraph для своей архитектуры рабочего процесса. Вы можете использовать LangGraph Studio для отладки и визуализации рабочего процесса в реальном времени.

#### Запуск LangGraph Studio локально

BoteWrite включает конфигурационный файл `langgraph.json`, который определяет структуру графа и зависимости для LangGraph Studio. Этот файл указывает на графы рабочего процесса, определенные в проекте, и автоматически загружает переменные окружения из файла `.env`.
...
BoteWrite поддерживает трассировку LangSmith, чтобы помочь вам отладить и контролировать ваши рабочие процессы. Чтобы включить трассировку LangSmith:
...
BoteWrite предоставляет настройку docker-compose для легкого запуска бэкенда и фронтенда вместе:
...
Следующие примеры демонстрируют возможности BoteWrite:
...
BoteWrite включает механизм "человек в контуре", который позволяет вам просматривать, редактировать и утверждать планы исследования перед их выполнением:
...
BoteWrite создан на основе невероятной работы сообщества открытого кода. Мы глубоко благодарны всем проектам и контрибьюторам, чьи усилия сделали BoteWrite возможным. Поистине, мы стоим на плечах гигантов.
...
- **[LangGraph](https://github.com/langchain-ai/langgraph)**: Их инновационный подход к оркестровке многоагентных систем сыграл решающую роль в обеспечении сложных рабочих процессов BoteWrite.
...
Сердечная благодарность основным авторам `BoteWrite`, чье видение, страсть и преданность делу вдохнули жизнь в этот проект:

- **[Daniel Walnut](https://github.com/hetaoBackend/)**
- **[Henry Li](https://github.com/magiccube/)**

Ваша непоколебимая приверженность и опыт стали движущей силой успеха BoteWrite. Мы считаем за честь иметь вас во главе этого путешествия.
